...
async function load(path: string) {
    if (fs.statSync(path).isDirectory()) for (const name of fs.readdirSync(path)) await load(`${path}/${name}`);
    else {
...
async function fetch(req: Request, internal: boolean) {
        ...
        if ((version !== "test" || Bun.env.DEBUG) && url.pathname.startsWith(`/${version}`)) {
            try {
                ...
                for (const [key, test] of Object.entries(options)) {
                    if (!internal && test.internal) continue;
                ...
                if (route.scope) {
                    ...
                    if (user!.scopes && !user!.scopes.includes("all"))
                        while (true) {
                            ...
                            if (!scope.includes("/")) throw [403, codes.MISSING_SCOPE, `Provided key does not have the required scope (${route.scope}).`];
                            scope = scope.replace(/\/[^\/]*$/, "");
                        }
                }
                ...
                for (const [key, value] of url.searchParams.entries()) search[key] = value;

                if (route.schema?.query && !route.schema.query(search))
                    throw [400, codes.INVALID_BODY, { message: "Invalid query arguments provided.", error: route.schema.query.errors }];

                const handle = handlers[version][method]?.[realpath!];
                if (!handle) throw [501, codes.NOT_IMPLEMENTED, "Not implemented."];

                const data = await handle({ req, params, search, body, user: user!, token: token! });
                ...
                else if (typeof error === "number") error = [error, -1, ""];
...
Bun.serve({ development: !!Bun.env.DEBUG, fetch: (req) => fetch(req, false) });
Bun.serve({ port: Bun.env.INTERNAL_PORT!, fetch: (req) => fetch(req, true) });
